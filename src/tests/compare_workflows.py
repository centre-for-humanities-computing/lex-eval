...
from lex_eval.metrics.metrics import (
    MetricsCalculator,
...
async def compare_workflows_on_query(
    connector: LexLLMConnector, query: str, workflow_ids: list[str]
):
    """
    ...
    if len(results) > 1:
        ...
        print("\n FAITHFULNESS:")
        print(
            f"  • With citations: {comparison['faithfulness']['pct_with_citations']:.0f}%"
        )
