...
class MetricsCalculator:
    ...
    def calculate_retrieval_metrics(sources: List[Dict[str, Any]]) -> RetrievalMetrics:
        ...
        if not sources:
            return RetrievalMetrics(
                num_sources=0, avg_source_length=0.0, source_diversity=0.0
            )
    ...
    def calculate_answer_metrics(
        response: str, sources: List[Dict[str, Any]]
    ) -> AnswerMetrics:
        """
        ...
        citation_count = sum(
            len(re.findall(pattern, response.lower())) for pattern in citation_patterns
        )
    ...
    def calculate_faithfulness_metrics(
        ...
        has_citations = bool(
            re.search(r"\[.*?\]|ifølge|artikel|kilde", response.lower())
        )
        ...
        claims_count = sum(
            1 for sent in sentences if re.search(claim_patterns, sent.lower())
        )

        return FaithfulnessMetrics(
            has_citations=has_citations, claims_count=claims_count
        )

    @classmethod
    def evaluate_response(
        cls, query: str, workflow_id: str, response: str, sources: List[Dict[str, Any]]
    ) -> EvaluationResult:
        """
...
def compare_workflows(results: List[EvaluationResult]) -> Dict[str, Any]:
    ...
    comparison = {
        ...
        "retrieval": {
            ...
            "avg_diversity": sum(r.retrieval.source_diversity for r in results)
            / len(results),
        },
        "answer": {
            "avg_length": sum(r.answer.answer_length for r in results) / len(results),
            "avg_citations": sum(r.answer.citation_count for r in results)
            / len(results),
            "avg_overlap": sum(r.answer.answer_source_overlap for r in results)
            / len(results),
        },
        "faithfulness": {
            "pct_with_citations": sum(r.faithfulness.has_citations for r in results)
            / len(results)
            * 100,
            "avg_claims": sum(r.faithfulness.claims_count for r in results)
            / len(results),
        },
    }
